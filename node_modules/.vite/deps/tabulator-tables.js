...
var Export = class extends Module {
  ...
  generateExportList(config, style, range2, colVisProp) {
    ...
    if (range2 === "range") {
      ...
      body = this.bodyToExportRows(this.rowLookup(range2), this.table.modules.selectRange.selectedColumns(true));
    } else {
      headers = this.config.columnHeaders !== false ? this.headersToExportRows(this.generateColumnGroupHeaders()) : [];
      body = this.bodyToExportRows(this.rowLookup(range2));
    }
    ...
    if (typeof range2 == "function") {
      ...
      switch (range2) {
        ...
        case "selected":
          ...
          break;
        case "range":
          rows2 = this.table.modules.selectRange.selectedRows();
          break;
        case "active":
        default:
  ...
  generateRowElement(row, setup, styles) {
    ...
    row.columns.forEach((col, i) => {
      if (col) {
        var cellEl = document.createElement("td"), column = col.component._column, table = this.table, index = table.columnManager.findColumnIndex(column), value = col.value, cellStyle;
        var cellWrapper = {
          modules: {},
          getValue: function() {
            return value;
...
var Range = class extends CoreFeature {
  ...
  ///////////////////////////////////
  layout() {
    var _vDomTop = this.table.rowManager.renderer.vDomTop, _vDomBottom = this.table.rowManager.renderer.vDomBottom, _vDomLeft = this.table.columnManager.renderer.leftCol, _vDomRight = this.table.columnManager.renderer.rightCol, top, bottom, left, right, topLeftCell, bottomRightCell;
    if (_vDomTop == null) {
      _vDomTop = 0;
    }
    if (_vDomBottom == null) {
...
var SelectRange = class extends Module {
  ...
  navigate(jump, expand, dir) {
    var moved = false, range2, rangeEdge, nextRow, nextCol, row, column;
    if (this.table.modules.edit && this.table.modules.edit.currentCell) {
      return false;
    }
  ...
  findJumpCellLeft(rowPos, colPos) {
    var row = this.getRowByRangePos(rowPos), cells = row.cells.filter((cell) => cell.column.visible), isStartingCellEmpty = !cells[colPos].getValue(), isLeftOfStartingCellEmpty = cells[colPos] ? !cells[colPos].getValue() : false, jumpCol = colPos, targetCells = this.rowHeader ? cells.slice(1, colPos) : cells.slice(0, colPos), nextCell = this.findJumpCell(targetCells, true, isStartingCellEmpty, isLeftOfStartingCellEmpty);
    if (nextCell) {
  ...
  findJumpCellRight(rowPos, colPos) {
    var row = this.getRowByRangePos(rowPos), cells = row.cells.filter((cell) => cell.column.visible), isStartingCellEmpty = !cells[colPos].getValue(), isRightOfStartingCellEmpty = cells[colPos + 1] ? !cells[colPos + 1].getValue() : false, jumpCol = colPos, nextCell = this.findJumpCell(cells.slice(colPos + 1, cells.length), false, isStartingCellEmpty, isRightOfStartingCellEmpty);
    if (nextCell) {
      jumpCol = nextCell.column.getPosition() - 1;
    }
    return jumpCol;
  ...
  findJumpCellUp(rowPos, colPos) {
    var column = this.getColumnByRangePos(colPos), cells = column.cells.filter((cell) => this.table.rowManager.activeRows.includes(cell.row)), isStartingCellEmpty = !cells[rowPos].getValue(), isTopOfStartingCellEmpty = cells[rowPos - 1] ? !cells[rowPos - 1].getValue() : false, jumpRow = rowPos, nextCell = this.findJumpCell(cells.slice(0, jumpRow), true, isStartingCellEmpty, isTopOfStartingCellEmpty);
    if (nextCell) {
  ...
  findJumpCellDown(rowPos, colPos) {
    var column = this.getColumnByRangePos(colPos), cells = column.cells.filter((cell) => this.table.rowManager.activeRows.includes(cell.row)), isStartingCellEmpty = !cells[rowPos].getValue(), isBottomOfStartingCellEmpty = cells[rowPos + 1] ? !cells[rowPos + 1].getValue() : false, jumpRow = rowPos, nextCell = this.findJumpCell(cells.slice(jumpRow + 1, cells.length), false, isStartingCellEmpty, isBottomOfStartingCellEmpty);
    if (nextCell) {
      jumpRow = nextCell.row.position - 1;
    }
    return jumpRow;
  ...
  autoScroll(range2, row, column) {
    var tableHolder = this.table.rowManager.element, rowHeader, rect, view, withinHorizontalView, withinVerticalView;
